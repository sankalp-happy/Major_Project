...
from phase2.runtime import MockRuntime
...
def test_phase23_orchestration_obligation_closure(tmp_path):
    analysis = analyze_phase1(REPO_ROOT)
    ...
    closed = report["obligations"]["closed"]
    assert any(item["target_name"] == "main" for item in closed)
    ...
    assert (tmp_path / "phase2" / "evaluation.json").exists()
